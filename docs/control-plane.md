...
The Services stream is deliberately a single `EventSource`: it sends one authoritative health snapshot when connected, then multiplexes compact `telemetry` events on the same SSE connection. By default, bounded telemetry updates every 2 seconds while the heavier cross-subsystem health probe runs every 10 seconds; keepalives are every 20 seconds. These cadences are configurable through `CPTR_MCP_SERVICES_TELEMETRY_INTERVAL_MS`, `CPTR_MCP_SERVICES_HEALTH_INTERVAL_MS`, and `CPTR_MCP_SERVICES_KEEPALIVE_INTERVAL_MS`. Telemetry contains aggregates only—request/DB latency and errors, command capacity, worker state, MCP traffic/RTT, host resource counters, and queue pressure. It never duplicates terminal output, prompt content, browser frames, or raw MCP event histories. The mobile UI consumes the compact delta directly and uses safe-area-aware gutters, 44-point-class touch targets, progressive disclosure, and deferred rendering for lower panels.

Process-level runtime probes are also decimated independently from hot request instrumentation. `CPTR_METRICS_PROCESS_SAMPLE_INTERVAL_MS` defaults to five seconds so a large `/proc/self/fd` directory is not rescanned on every dashboard read. The system sampler keeps cheap `/proc` CPU/memory/disk/network counters at its normal cadence while `ps`/GPU probes default to every ten seconds through `CPTR_MCP_SYSTEM_METRICS_EXPENSIVE_INTERVAL_MS`. Command observability uses a read-only registry projection; process reconciliation remains owned by the normal reaper and explicit maintenance paths.

Bands are `healthy | moderate | unhealthy` and are derived only from re-measured probes (backend readiness/metrics, permanent-worker supervision, plugin identity/contract, extension device connectivity, MCP diagnostics/traffic stores). Maintain jobs run fixed playbooks only—no free-form shell and no AI/model call. **Stabilize All** is a bounded deterministic convergence loop: it runs the dependency-ordered service playbooks, re-probes authoritative health, and performs at most one second pass for only the services that remain non-healthy. The job exposes `system_status = STABLE | DEGRADED | ACTION_REQUIRED | FAILED` plus `pass_count`; the legacy `succeeded | partial | failed` job status remains for API compatibility. Backend maintenance performs a non-blocking SQLite `wal_checkpoint(PASSIVE)`, command-session/reservation reconciliation, normal completed-session reaping, and a safe background-worker reconcile. MCP transport maintenance expires stale traffic sessions before re-probing its bounded stores. Operations that Computer cannot perform safely with its current ownership information, such as force-releasing a Chrome debugger lease or refreshing ChatGPT's frozen tool snapshot, remain explicitly `report_only` rather than pretending to repair them. Missing or unreachable signals fail closed (never invent healthy).

The FastAPI lifespan owns one model-free background-worker watchdog for the permanent command reaper, Workbench reaper, event-loop monitor, memory maintenance worker, automation scheduler, and timer worker. Each loop emits a cheap heartbeat; long poll sleeps are pulsed so normal inactivity is not mistaken for a stall. A missing or exited worker is restarted automatically within a bounded restart circuit (three attempts in a five-minute window by default). A live task with a stale heartbeat is reported as `stalled` and degrades health, but is **not** force-cancelled. Ordinary health snapshots are read-only and never restart workers; reconciliation occurs only in the watchdog supervisor or an explicit Backend/Stabilize maintenance playbook. This preserves active commands, Chrome ownership, Factory work, and user memory rather than destroying workload to manufacture a green status.

Plugin identity is read from the companion plugin's authoritative `/plugin/update` manifest rather than from MCP `clientInfo` (which describes the ChatGPT client). Configure either `CPTR_MCP_PLUGIN_UPDATE_URL` directly or `CPTR_MCP_PLUGIN_BASE_URL` and Computer will append `/plugin/update`. Regular health snapshots rate-limit this cross-repo probe with `CPTR_MCP_PLUGIN_PROBE_INTERVAL_SECONDS` (15 seconds by default); an explicit plugin/all Maintain job bypasses that cache and forces a fresh manifest probe before its mandatory post-check. Cross-repo expectations can be advanced without code changes through `CPTR_MCP_EXPECTED_CONTRACT_VERSION` and `CPTR_MCP_EXPECTED_TOOL_COUNT`. A manifest `refresh_required=true` is an operator-action advisory, not a plugin runtime failure: when identity, contract, and tool count are current, the Plugin service and runtime aggregate remain `healthy`, while **Stabilize All** returns `ACTION_REQUIRED` with a healthy post-band until the ChatGPT host refresh action is handled. This prevents static release metadata from permanently degrading runtime health while still refusing to claim that Computer can refresh ChatGPT's frozen action snapshot itself.
