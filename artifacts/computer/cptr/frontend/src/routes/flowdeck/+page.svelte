...
FLOWDECK_DRAFT_STORAGE_KEY,
	...
	function describeState(kind: RunKind): string {
		...
		if (kind === 'active')
			return 'FlowDeck is coordinating this objective. This view refreshes automatically.';
		return 'Waiting for a truthful state from the orchestration service.';
	}

	const safeRunIdentifier = /^[A-Za-z0-9_-]{1,128}$/;
	const sensitiveKey =
		/(?:credential|password|secret|token|authorization|cookie|process[_ -]?output|stderr|stdout|stack|exception|traceback|filesystem|path|command|environment|owner|run[_ -]?id|api[_ -]?key)/i;
	const sensitiveText =
	...
	function waitForDiagnosticsRetry(milliseconds: number): Promise<void> {
	...
	async function startOrchestration(event: SubmitEvent) {
		...
		try {
			...
			startError = 'Controlled orchestration could not be started. Nothing was launched.';
		} finally {
			isStarting = false;
		}
	}
...
<div class="flowdeck-shell">
	<header class="flowdeck-header">
		<div class="header-brand">
			<div class="brand-mark" aria-hidden="true">
				<Icon name="gateway" size={17} strokeWidth={1.8} />
			</div>
			<div>
		...
		<div class="header-status" aria-live="polite">
			<span class:status-pulse={connectionState === 'connected' && !isTerminal} class="status-dot"
			></span>
	...
	<section
		...
		data-testid="fdx-diagnostics-panel"
	...
	>
		<div class="diagnostics-heading">
			<div>
				...
				<p>
					Safe lifecycle categories across your FlowDeck runs. All listed failures used native
					fallback.
				</p>
			</div>
			...
			<div class="diagnostics-list">
				{#each diagnostics as diagnostic}
					<a
						...
						href={`/flowdeck?run_id=${encodeURIComponent(diagnostic.run_id)}`}
					>
						...
						<span class="diagnostic-outcome"
							>Native fallback · {categoryLabel(diagnostic.run_status)}</span
						>
						<time datetime={new Date(diagnostic.created_at).toISOString()}
							>{formatTime(diagnostic.created_at)}</time
						>
					</a>
				{/each}
			</div>
			...
			<section class="welcome-grid">
				<div class="welcome-copy">
					...
					<div class="trust-notes">
						...
						<div>
							<Icon name="refresh" size={15} /><span>State rehydrates after reconnect</span>
						</div>
					</div>
				</div>
				...
				<form class="composer-card" action="javascript:void(0)" onsubmit={startOrchestration}>
					...
					<div class="composer-foot">
						...
						<button
							class="start-button"
							type="submit"
							disabled={isStarting || !workspace || !objective.trim()}
						>
							{#if isStarting}
					...
					{#if !isTerminal && runKind !== 'manual'}
						{#if confirmCancel}
							<div class="cancel-confirm" role="alertdialog" aria-label="Confirm cancellation">
								...
								<button class="cancel-dismiss" onclick={() => (confirmCancel = false)}
									>Keep running</button
								>
							</div>
						{:else}
				...
				<div class="reconnect-banner" role="alert">
					<div>
						<Icon name="refresh" size={16} /><span
							><strong>State refresh interrupted.</strong> {pollError}</span
						>
					</div>
					...
					<div class="state-card state-{runKind}">
						<div class="state-card-top">
							...
							<div class="state-connection">
								<span
									class:status-pulse={connectionState === 'connected' && !isTerminal}
									class="status-dot"
								></span>
								<span>{reconnectLabel}</span>
							</div>
						...
						<div class="state-meta">
							...
							<div>
								<span>Last checked</span><strong
									>{lastCheckedAt ? formatTime(lastCheckedAt) : 'Awaiting first read'}</strong
								>
							</div>
							<div>
								<span>Backend status</span><strong>{safeStatusLabel(getStatus(run))}</strong>
							</div>
						</div>
					</div>
					...
					<article class="activity-card">
								...
								{#each activityItems(run) as item}
									<div class="activity-item">
										...
										<div class="activity-item-copy">
											<strong>{eventTitle(item)}</strong>{#if eventDetail(item)}<span
													>{eventDetail(item)}</span
												>{/if}
										</div>
						...
						{:else}
							<div class="empty-telemetry">
								<Icon name="clock" size={18} /><span>No activity has been reported yet.</span>
							</div>
						{/if}
					</article>
				</div>
				...
				<aside class="run-side">
					<article class="evidence-card">
						...
						{:else}
							<div class="not-reported"><span></span>Not reported</div>
							<p class="side-note">
								Evidence will appear here when the orchestration backend returns it.
							</p>
						{/if}
					</article>
					<div class="reconnect-card">
						...
						<div>
							...
							<p>
								This run is owned by this browser session. Reloading reads {runId} again; it never starts
								a second run.
							</p>
						</div>
					</div>
...
<style>
	...
	.flowdeck-shell {
		...
		background:
			radial-gradient(
				circle at 86% 10%,
				color-mix(in oklab, var(--fd-teal) 9%, transparent),
				transparent 26rem
			),
			var(--app-bg);
		color: var(--fd-ink);
	...
	.status-pulse {
		...
		box-shadow: 0 0 0 4px color-mix(in oklab, var(--fd-teal) 15%, transparent);
	}
	...
	.diagnostic-filter span {
		color: var(--fd-muted);
		font-size: 0.66rem;
	}
	.diagnostic-filter select {
		width: 180px;
		height: 34px;
		padding: 0 0.5rem;
	...
	.diagnostics-retry:hover:not(:disabled) {
		border-color: color-mix(in oklab, var(--fd-coral) 45%, transparent);
		background: color-mix(in oklab, var(--fd-coral) 8%, transparent);
	}
	.diagnostics-list {
		display: grid;
		gap: 0.45rem;
	...
	.diagnostic-row:hover {
		border-color: color-mix(in oklab, var(--fd-teal) 50%, transparent);
		background: color-mix(in oklab, var(--fd-teal) 6%, transparent);
	}
	.diagnostic-category {
		color: var(--fd-coral);
		font-weight: 700;
	...
	.composer-card {
		...
		border: 1px solid var(--fd-line);
		border-radius: 19px;
		background: linear-gradient(145deg, var(--fd-panel-strong), var(--fd-panel));
		box-shadow: 0 22px 70px color-mix(in oklab, var(--fd-teal) 8%, transparent);
	}
	.composer-topline {
		display: flex;
		justify-content: space-between;
	...
	.composer-orbit {
		...
		height: 62px;
		margin-top: -0.25rem;
		border: 1px solid color-mix(in oklab, var(--fd-teal) 34%, transparent);
		border-radius: 50%;
		opacity: 0.8;
	}
	...
	.composer-orbit::after {
		position: absolute;
		inset: 8px;
		border: 1px solid color-mix(in oklab, var(--fd-teal) 30%, transparent);
		border-radius: 50%;
		content: '';
	}
	.composer-orbit::after {
		inset: 18px;
		background: var(--fd-teal);
		box-shadow: 0 0 0 5px color-mix(in oklab, var(--fd-teal) 13%, transparent);
	}
	.composer-orbit span {
		position: absolute;
		top: 3px;
	...
	.composer-orbit span:nth-child(2) {
		top: 41px;
		left: 4px;
		background: var(--fd-coral);
	}
	.composer-orbit span:nth-child(3) {
		top: 16px;
		left: 50px;
	}
	label {
	...
	select:focus,
	textarea:focus {
		border-color: color-mix(in oklab, var(--fd-teal) 68%, transparent);
		box-shadow: 0 0 0 3px color-mix(in oklab, var(--fd-teal) 13%, transparent);
	}
	...
	.inline-error {
		color: var(--fd-coral);
		background: color-mix(in oklab, var(--fd-coral) 10%, transparent);
	}
	.disabled-note {
		color: var(--fd-amber);
		background: color-mix(in oklab, var(--fd-amber) 11%, transparent);
	}
	.workspace-empty {
		display: flex;
		align-items: center;
	...
	.cancel-button {
		border-color: color-mix(in oklab, var(--fd-coral) 38%, transparent);
		color: var(--fd-coral);
	}
	.cancel-button:hover {
		background: color-mix(in oklab, var(--fd-coral) 8%, transparent);
	...
	.cancel-confirm {
		...
		gap: 0.4rem;
		padding: 0.3rem;
		border: 1px solid color-mix(in oklab, var(--fd-coral) 35%, transparent);
		border-radius: 9px;
		background: color-mix(in oklab, var(--fd-coral) 7%, var(--app-bg));
		font-size: 0.68rem;
	}
	.cancel-confirm span {
	...
	.reconnect-banner {
		...
		padding: 0.8rem 1rem;
		border: 1px solid color-mix(in oklab, var(--fd-amber) 36%, transparent);
		border-radius: 10px;
		background: color-mix(in oklab, var(--fd-amber) 9%, transparent);
		color: var(--fd-amber);
		font-size: 0.71rem;
	}
	.reconnect-banner > div {
	...
	.reconnect-card {
		border: 1px solid var(--fd-line);
		border-radius: 14px;
		background: color-mix(in oklab, var(--fd-panel) 75%, transparent);
	}
	.state-card {
		overflow: hidden;
	}
	...
	.state-icon {
		...
		place-items: center;
		border-radius: 11px;
		background: color-mix(in oklab, var(--fd-teal) 14%, transparent);
		color: var(--fd-teal);
	}
	.state-failure .state-icon {
		background: color-mix(in oklab, var(--fd-coral) 13%, transparent);
		color: var(--fd-coral);
	}
	.state-cancelled .state-icon {
		background: color-mix(in oklab, var(--fd-amber) 14%, transparent);
		color: var(--fd-amber);
	}
	.state-manual .state-icon {
		background: color-mix(in oklab, var(--fd-amber) 14%, transparent);
		color: var(--fd-amber);
	}
	.state-card h2 {
	...
	.state-meta {
		...
		background: color-mix(in oklab, var(--app-bg) 25%, transparent);
	}
	.state-meta span {
		display: block;
	...
	.telemetry-card {
		...
		border: 1px solid var(--fd-line);
		border-radius: 12px;
		background: color-mix(in oklab, var(--fd-panel) 70%, transparent);
	}
	.telemetry-label {
		display: block;
		color: var(--fd-faint);
	...
	.reconnect-card-icon {
		...
		place-items: center;
		border-radius: 8px;
		background: color-mix(in oklab, var(--fd-teal) 12%, transparent);
		color: var(--fd-teal);
	}
