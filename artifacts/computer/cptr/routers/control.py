...
async def _user(request: Request, scope: str) -> str:
    if os.environ.get("CPTR_CONTROL_PLANE_ENABLED", "false").lower() not in (
        "true",
        "1",
        "yes",
        "on",
    ):
        raise HTTPException(status_code=404, detail="control plane is unavailable")
    try:
...
async def _server_model(
    request: Request, workspace: Workspace, compatibility_model: str = ""
) -> str:
    """Resolve the model through FlowDeck policy, never from client input."""
    workspace_path = getattr(workspace, "path", None)
    if not workspace_path:
        # Direct route-unit tests use a sentinel workspace object. Production
        # requests always resolve a persisted Workspace with a canonical path.
        return compatibility_model
    from cptr.routers.flowdeck import _resolve_model

    target, _ = await _resolve_model(request, workspace_path, request.app.state)
    if target.kind != "api":
        raise HTTPException(status_code=503, detail="the CPTR API model is unavailable")
    return target.runtime_model
...
async def create_task(request: Request, body: TaskCreateRequest):
    ...
    model_id = await _server_model(request, workspace, body.model_id)
