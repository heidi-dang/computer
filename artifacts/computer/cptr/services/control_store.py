...
class SqlSupervisorStore:
    ...
    def __init__(self) -> None:
        self._flowdeck_leases: dict[str, tuple[str, str, int]] = {}
    ...
    async def claim_workspace(self, workspace_id: str, monitor_id: str) -> bool:
        ...
        if workspace is None:
            return False
        owner = f"control-supervisor:{monitor_id}"
        lease = await DurableFlowDeck(get_session_factory()).acquire_workspace_lease(
        ...
        if lease is None:
            return False
        self._flowdeck_leases[monitor_id] = (workspace.path, owner, lease.epoch)
        return True

    async def release_workspace(self, workspace_id: str, monitor_id: str) -> None:
        lease = self._flowdeck_leases.pop(monitor_id, None)
        if lease is None:
            return
        workspace, owner, epoch = lease
        await DurableFlowDeck(get_session_factory()).release_workspace_lease(
            workspace=workspace,
            owner=owner,
            epoch=epoch,
        )
