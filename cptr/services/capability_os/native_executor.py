...
class NativeActionExecutor:
    ...
    async def _reserve_bytes(self, lease: CapabilityLease, *, read: int = 0, written: int = 0) -> None:
    ...
    def _lease_context(lease: CapabilityLease) -> tuple[str, str]:
        ...
        if not user_id or not workspace_id:
            raise NativeActionDenied("native action lease lacks server-bound user/workspace context")
    ...
    async def invoke(
        ...
        *,
        ...
        if workspace.user_id != user_id or workspace.workspace_id != workspace_id:
            raise NativeActionDenied("native workspace resolver returned mismatched authority context")
        await self._reserve_call(lease)
        return await handler(workspace, dict(inputs), lease, int(timeout_ms))

    async def _list(
        self, workspace: NativeWorkspaceContext, inputs: dict[str, Any], lease: CapabilityLease, timeout_ms: int
    ) -> ActionResult:
        full, relative = self._relative(workspace.root, inputs.get("path", "."))
        ...
        result = await self._runtime.list_tree_entries_as(
            workspace.identity, str(full), recursive, offset, limit
        )
    ...
    async def _read_text(
        ...
        full, relative = self._relative(workspace.root, inputs["path"])
        self._require_permission(
            lease,
            action="filesystem.read",
            resource=self._resource(workspace.workspace_id, relative),
        )
        stat = await self._runtime.stat_as(workspace.identity, str(full))
        if stat.get("type") != "file":
            raise NativeActionDenied("native read requires a file")
        ...
        if size > read_limit:
            raise NativeActionDenied("native read exceeds requested byte limit")
        await self._reserve_bytes(lease, read=size)
        data = await self._runtime.read_text_file_as(workspace.identity, str(full), read_limit)
        if data.get("binary"):
    ...
    async def _write_text(
        ...
        if exists:
            if stat.get("type") != "file":
                raise NativeActionDenied("native write target is not a file")
            if not overwrite:
                raise NativeActionDenied("native write requires overwrite=true for an existing file")
            current_size = int(stat.get("size") or 0)
            if current_size > _MAX_TEXT_BYTES:
                raise NativeActionDenied("existing file is too large for safe native overwrite")
            current = await self._runtime.read_text_file_as(
                workspace.identity, str(full), _MAX_TEXT_BYTES
            )
            if current.get("binary"):
                raise NativeActionDenied("binary files cannot be overwritten by native write")
            current_text = str(current.get("content") or "")
            current_digest = hashlib.sha256(current_text.encode("utf-8")).hexdigest()
            if expected is not None and current_digest != expected:
                raise NativeActionDenied("native write stale-hash precondition failed")
        elif expected is not None:
            raise NativeActionDenied("expectedSha256 cannot be used for a missing file")
        await self._reserve_bytes(lease, written=len(encoded))
        await self._runtime.write_file_as(workspace.identity, str(full), content)
        ...
        return ActionResult(
            ...
            verification_passed=True,
            metadata={"actionClass": "native", "bytesWritten": len(encoded), "timeoutMs": timeout_ms},
        )
