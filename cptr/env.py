...
COMMAND_IDEMPOTENCY_RESERVATION_TTL_SECONDS = max(
    5, min(_env_int("CPTR_COMMAND_IDEMPOTENCY_RESERVATION_TTL_SECONDS", 30), 300)
)
...
DIRECT_WORKER_MAX_PER_WORKSPACE = max(1, _env_int("CPTR_DIRECT_WORKER_MAX_PER_WORKSPACE", 32))
...
# ── Capability OS sandbox broker ────────────────────────────
# Generated code never runs in the CPTR web process. The application talks to
# a separately installed/root-owned broker through this Unix-domain socket.
CAPABILITY_OS_SANDBOX_SOCKET = Path(
    os.environ.get("CPTR_CAPABILITY_OS_SANDBOX_SOCKET", "/run/cptr-sandbox-broker.sock")
)
CAPABILITY_OS_BUILD_MAX_CPU_MILLIS = max(
    100, _env_int("CPTR_CAPABILITY_OS_BUILD_MAX_CPU_MILLIS", 60_000)
)
...
CAPABILITY_OS_BUILD_MAX_OUTPUT_BYTES = max(
