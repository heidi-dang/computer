<script lang="ts">
	...
	async function refreshAll() {
		...
		error = '';
		try {
			const [repoResult, catalogResult, instructionResult, historyResult, envResult, contextResult, checkpointResult, healthResult] =
				await Promise.all([
					...
					getWorkspaceEnvironmentProfiles(workspace.workspace_id),
					getWorkspaceContext(workspace.workspace_id),
					getWorkspaceCheckpoints(workspace.workspace_id),
					getWorkspaceHealth(workspace.workspace_id)
				]);
			...
			contextData = contextResult as unknown as Record<string, unknown>;
...
<Modal {onclose} class="w-full max-w-5xl mx-0 sm:mx-4 max-h-[92dvh]">
	<div class="flex min-h-[36rem] max-h-[92dvh] flex-col overflow-hidden">
		<header class="flex items-start justify-between gap-4 border-b border-[var(--app-border)] px-5 py-4">
			<div class="min-w-0">
				<div class="flex items-center gap-2">
					...
					{#if health && typeof health.health_band === 'string'}
						<span class="rounded-full border border-[var(--app-border)] px-2 py-0.5 text-[11px] uppercase tracking-wide">
							{health.health_band}
						</span>
					{/if}
				</div>
		...
		<div class="flex min-h-0 flex-1 flex-col sm:flex-row">
			<nav class="flex shrink-0 gap-1 overflow-x-auto border-b border-[var(--app-border)] p-2 sm:w-44 sm:flex-col sm:border-b-0 sm:border-r">
				{#each tabs as tab}
					<button
						class="whitespace-nowrap rounded-lg px-3 py-2 text-left text-sm transition-colors {activeTab === tab.id
			...
			<main class="min-h-0 flex-1 overflow-y-auto p-5">
				{#if loading}
					<div class="flex min-h-48 items-center justify-center text-sm text-[var(--app-muted-fg)]">
						Loading Workspace OS…
					</div>
				{:else if error}
					<div class="rounded-xl border border-red-500/30 bg-red-500/5 p-4">
						<p class="text-sm text-red-500">{error}</p>
						<button class="mt-3 rounded-lg border border-[var(--app-border)] px-3 py-1.5 text-sm" onclick={refreshAll}>
							Retry
						</button>
					</div>
				{:else if activeTab === 'general'}
					<section class="space-y-5">
						<div>
							...
							<p class="mt-1 text-sm text-[var(--app-muted-fg)]">Stable Workspace identity; filesystem path remains a compatibility execution root.</p>
						</div>
						<label class="block">
						...
						<div class="rounded-xl border border-[var(--app-border)] p-3 text-xs text-[var(--app-muted-fg)]">
							<div><strong>Workspace ID:</strong> {workspace.workspace_id}</div>
							<div class="mt-1 break-all"><strong>Compatibility path:</strong> {workspace.path}</div>
						</div>
						<button class="workspace-primary" disabled={busy || !name.trim()} onclick={saveGeneral}>Save identity</button>
					</section>
				{:else if activeTab === 'repositories'}
					<section class="space-y-4">
						<div>
							<h3 class="text-base font-semibold">Repositories</h3>
							<p class="mt-1 text-sm text-[var(--app-muted-fg)]">Explicit logical repository membership. Worker worktrees remain ephemeral.</p>
						</div>
						<div class="flex flex-col gap-2 sm:flex-row">
							<select class="workspace-field flex-1" bind:value={selectedRepository}>
								...
								{#each availableRepositories as repo}
									<option value={repo.repository_id}>{repo.name} · {repo.canonical_remote_identity}</option>
								{/each}
							</select>
							<button class="workspace-primary" disabled={busy || !selectedRepository} onclick={addRepository}>Add</button>
						</div>
						{#if repositories.length === 0}
							<p class="rounded-xl border border-dashed border-[var(--app-border)] p-5 text-sm text-[var(--app-muted-fg)]">No explicit repository membership yet.</p>
						{/if}
							...
							<div class="rounded-xl border border-[var(--app-border)] p-4">
								<div class="flex flex-wrap items-start justify-between gap-3">
									<div class="min-w-0">
										...
										<div class="mt-1 break-all text-xs text-[var(--app-muted-fg)]">{repo.canonical_remote_identity}</div>
									...
									<div class="flex gap-2">
										{#if !repo.primary}
											<button class="workspace-secondary" disabled={busy} onclick={() => setPrimary(repo)}>Make primary</button>
										{/if}
										<button class="workspace-danger" disabled={busy} onclick={() => removeRepository(repo)}>Remove</button>
									</div>
								</div>
				...
				{:else if activeTab === 'instructions'}
					<section class="space-y-4">
						<div>
							...
							<p class="mt-1 text-sm text-[var(--app-muted-fg)]">Authoritative project instructions, loaded before Workspace memory.</p>
						</div>
						<textarea class="workspace-field min-h-64 resize-y font-mono text-xs" bind:value={instructionText}></textarea>
						<div class="flex items-center justify-between gap-3">
							<span class="text-xs text-[var(--app-muted-fg)]">Current version: {instruction?.version ?? 0}</span>
							<button class="workspace-primary" disabled={busy} onclick={saveInstructions}>Save new version</button>
						</div>
						<div>
							...
							<div class="space-y-2">
								{#each instructionHistory.slice(0, 10) as item}
									<div class="rounded-lg border border-[var(--app-border)] p-3 text-xs">
										<div class="font-medium">v{item.version} {item.is_current ? '· current' : ''}</div>
										<div class="mt-1 text-[var(--app-muted-fg)]">{item.change_summary || item.content_hash}</div>
									</div>
								{/each}
							</div>
						</div>
				...
				{:else if activeTab === 'environment'}
					<section class="space-y-4">
						<div>
							...
							<p class="mt-1 text-sm text-[var(--app-muted-fg)]">Versioned runtime configuration. Secret material is never returned here.</p>
						</div>
						<div class="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
							<input class="workspace-field" placeholder="Profile name" bind:value={newEnvironmentName} />
							<input class="workspace-field" placeholder="Runtime profile" bind:value={newRuntimeProfile} />
							<button class="workspace-primary" disabled={busy || !newEnvironmentName.trim()} onclick={createEnvironment}>Create</button>
						</div>
						{#each environmentProfiles as profile}
							<div class="rounded-xl border border-[var(--app-border)] p-4">
								...
								<div class="mt-3 flex gap-2">
									...
									/>
									<button class="workspace-secondary" disabled={busy} onclick={() => saveEnvironmentTarget(profile)}>Save target</button>
								</div>
							</div>
						{/each}
					</section>
				{:else if activeTab === 'memory'}
					<section class="space-y-4">
						<div>
							...
							<p class="mt-1 text-sm text-[var(--app-muted-fg)]">Stable UUID-scoped Memory Core context automatically loaded for this Workspace.</p>
						</div>
						<div class="grid gap-3 sm:grid-cols-3">
							...
							<div class="workspace-stat">
								...
								<strong class="truncate text-xs">{String((contextData?.context as Record<string, unknown> | undefined)?.snapshot_id ?? 'ready')}</strong>
							</div>
						</div>
						<pre class="workspace-code">{pretty((contextData?.context as Record<string, unknown> | undefined)?.diagnostics ?? [])}</pre>
					</section>
				{:else if activeTab === 'checkpoints'}
					<section class="space-y-4">
						<div>
							<h3 class="text-base font-semibold">Checkpoints</h3>
							<p class="mt-1 text-sm text-[var(--app-muted-fg)]">Memory/execution recovery checkpoints. Restoring one never performs Git rollback.</p>
						</div>
						<div class="text-xs text-[var(--app-muted-fg)]">Memory namespace version: {checkpointMemoryVersion}</div>
						{#if checkpoints.length === 0}
							<p class="rounded-xl border border-dashed border-[var(--app-border)] p-5 text-sm text-[var(--app-muted-fg)]">No checkpoints yet.</p>
						{/if}
						{#each checkpoints as checkpoint}
							<div class="rounded-xl border border-[var(--app-border)] p-4">
								<div class="flex items-center justify-between gap-3">
									<div class="font-medium">#{checkpoint.version} · {checkpoint.stage}</div>
									<div class="text-xs text-[var(--app-muted-fg)]">{new Date(checkpoint.created_at_ms).toLocaleString()}</div>
								</div>
								<div class="mt-1 text-xs text-[var(--app-muted-fg)]">
					...
					<section class="space-y-4">
						<div class="flex items-start justify-between gap-3">
							<div>
								...
								<p class="mt-1 text-sm text-[var(--app-muted-fg)]">Read-only classification plus conservative state reconciliation. It never deletes worktrees, branches, or uncommitted changes.</p>
							</div>
							<button class="workspace-secondary" disabled={busy} onclick={runReconcile}>Reconcile</button>
						</div>
						<div class="grid gap-3 sm:grid-cols-2">
							<div class="workspace-stat">
								<span>Health</span>
								<strong>{String(health?.health_band ?? 'unknown')}</strong>
							</div>
							<div class="workspace-stat">
								<span>Git repository</span>
								<strong>{health?.is_git_repo ? 'Yes' : 'No'}</strong>
							</div>
						</div>
						<pre class="workspace-code">{pretty(health?.summary ?? {})}</pre>
						<pre class="workspace-code">{pretty(health?.diagnostics ?? [])}</pre>
					</section>
				{/if}
			</main>
		</div>
