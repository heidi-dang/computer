...
class MigrationConvergenceTests(unittest.TestCase):
    ...
    def test_merged_history_has_one_head_and_supports_fresh_and_both_legacy_lineages(self):
        ...
        self.assertEqual(script.get_heads(), ["0026"])
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(self._version(fresh), "0026")
            self.assertTrue({"factory_runs", "browser_devices"} <= self._tables(fresh))
            ...
            self.assertEqual(self._version(factory), "0026")
            self.assertIn("browser_devices", self._tables(factory))
            ...
            self.assertEqual(self._version(legacy_main), "0026")
            self.assertTrue(
                {
                    "factory_runs",
